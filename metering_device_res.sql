--
--ќписание полей таблиц в комментах к ним
--
...
-- Comments
comment on table metering_device                          is 'ѕрибор учета'; 
...
-- Comments
comment on table spr_metering_device_type                is '“ип прибора учета'; 

comment on column spr_metering_device_type.id            is 'ID типа прибора учета'; 
comment on column spr_metering_device_type.m_d_type_name is 'Ќаименование типа прибора учета'; 

-- Inserts
insert into spr_metering_device_type values (1, 'холодна€ вода');
insert into spr_metering_device_type values (2, 'гор€ча€ вода');
...
-- Comments
comment on table spr_metering_device_placement                     is 'ћесто размещени€'; 

comment on column spr_metering_device_placement.id                 is 'ID места размещени€'; 
comment on column spr_metering_device_placement.m_d_placement_name is 'Ќаименование места размещени€'; 
...
--       p_rep_period_mon     - отчетный период (мес€ц), 
...
--       m_d_placement_name   -	место размещени€.
--
-- ¬ызов: select * from f_get_m_device_by_placement(1, 11, 2024);
--
